...
\]
with leading asymptotic $48\pi^2 s_1 s_2/R^2$ as $s_i \ll R$. The linear-in-$s_i$ vanishing controls the cross-term in the asymptotic-product collar geometry, and Theorem~\ref{thm:M_k-collar-conditional} extracts the codimension-$j$ collar essential-spectrum bottom $4j/r^2$ via a Weyl quasi-mode argument on the product of $j$ hyperbolic ends.
...
Fix a separation scale $\rho_0 > 0$. Let $\cM_k(S^4_r)$ be the $k$-instanton moduli for $\mathrm{SU}(2)$ on $S^4_r$, equipped with the $L^2$ metric, and let $U_\epsilon^{(j)} \subset \cM_k$ denote the tubular neighborhood of the Uhlenbeck stratum at which exactly $j$ instantons have scale parameter below $\epsilon$, and whose $j$ sub-$\epsilon$ centers stay pairwise separated by at least $\rho_0$ and at distance at least $\rho_0$ from the other $k-j$ centers (the codimension-$j$ collar with separation floor $\rho_0$). This separation floor excises the deeper collision strata, on which the product structure degenerates; it is the region where the geometry is asymptotically a Riemannian product. Then the collar-\emph{localized} essential-spectrum bottom satisfies
\[
...
\end{equation}
for some constant $C$ depending only on the geometry of $S^4_r$ and on the separation floor $\rho_0$ (the inter-bubble separation $R$ is bounded below by $\rho_0$ throughout $U_\epsilon^{(j)}$ by definition of the collar, which is exactly what the separation floor guarantees). By Proposition~\ref{prop:M1-iso}, each individual bubble factor is, in the limit $s_i \to 0$, isometric to the end of $\bbH^5_r$ (a conformally compact, asymptotically hyperbolic end with compact $S^4$ cross-sections).

In fact \eqref{eq:metric-product-bound} extends to a $C^2$ comparison of the perturbation tensor itself. Writing $h := g_{U_\epsilon^{(j)}} - g_{\mathrm{prod}}$ and denoting by $\nabla^{\mathrm{prod}}$ the Levi-Civita connection of the product metric, for $\epsilon$ small enough one has, uniformly on the ends of $U_\epsilon^{(j)}$:
\begin{equation}\label{eq:metric-product-bound-C2}
|h|_{g_{\mathrm{prod}}} \;\le\; C\epsilon|\log\epsilon|, \qquad
|\nabla^{\mathrm{prod}} h|_{g_{\mathrm{prod}}} \;\le\; C\,\frac{\epsilon|\log\epsilon|^{2}}{r_{\mathrm{cusp}}}, \qquad
|(\nabla^{\mathrm{prod}})^{2} h|_{g_{\mathrm{prod}}} \;\le\; C\,\frac{\epsilon|\log\epsilon|^{3}}{r_{\mathrm{cusp}}^{2}},
\end{equation}
...
The ends of $\bbH^5_r$ have purely continuous spectrum with no $L^2$ ground state. We build the quasi-mode in the geodesic-polar model of \S\ref{sec:mourre} (in which each bubble end has \emph{compact} $S^4$ cross-section, so the norms below are finite). In the transverse-constant ($\ell=0$) sector the half-density-reduced radial operator on the $i$-th end is $H_0^{(i)} = -\partial_{\rho_i}^2 + 4/r^2 + 2/(r^2\sinh^2(\rho_i/r))$ on $L^2((0,\infty),d\rho_i)$; its threshold generalized eigenfunction $g_\infty(\rho_i)$ tends to a constant as $\rho_i\to\infty$ (the centrifugal term being short-range). Choose smooth cutoffs $\chi_n$ supported in $[\rho_0, \rho_0 + n]$ with $\chi_n\equiv 1$ on $[\rho_0+1, \rho_0+n-1]$ and $\|\chi_n'\|_\infty + \|\chi_n''\|_\infty \le C$, and set (transverse-constant on each $S^4$ factor, and half-density-reduced)
\[
\Phi^{(j)}_n \;=\; \prod_{i=1}^j g_\infty(\rho_i)\,\chi_n(\rho_i).
\]
Direct calculation on the \emph{exact} product model gives $(-\Lap_{\mathrm{prod}} - 4j/r^2)\Phi^{(j)}_n = \sum_i O(\chi_n'(\rho_i)) + O(\chi_n''(\rho_i))$, and $\|\chi_n\|_{L^2(d\rho)} \sim \sqrt{n}$, $\|\chi_n'\|_{L^2}, \|\chi_n''\|_{L^2} = O(1)$ (the $S^4$ cross-section integrates to the finite factor $\mathrm{vol}(S^4)^j$), so
\[
...
\]
The second term is $O(\epsilon|\log\epsilon|)$: the reduced operator is order-one in $\rho_i$, so $\|\nabla^2\Phi^{(j)}_n\|_{L^2}^2 = O(\|\chi_n\|_{L^2}^2 + \|\chi_n'\|_{L^2}^2 + \|\chi_n''\|_{L^2}^2) = O(n+1)$ while $\|\Phi^{(j)}_n\|_{L^2}^2 = O(n)$, giving ratio $O(1)$. Choosing $n = n(\epsilon)\to\infty$ slowly enough that the first term is $o(1)$ while $\epsilon|\log\epsilon|\to 0$ makes both terms vanish. Hence $4j/r^2 \in \sigma_{\mathrm{ess}}(-\Lap_{U_\epsilon^{(j)}})$ for $\epsilon$ small enough, giving $\limsup_\epsilon \lambda_0^{\mathrm{ess}}(U_\epsilon^{(j)}) \le 4j/r^2$.
...
Let $U_\epsilon^{(j)} \subset \cM_k(S^4_r)$ be the codimension-$j$ Uhlenbeck collar as in Theorem~\ref{thm:M_k-collar-conditional}. Define the \emph{first-channel window}
\[
W_j := \begin{cases}\bigl(4j/r^2,\; 4(j+1)/r^2\bigr) & 1 \le j < k,\\[2pt] \bigl(4k/r^2,\; \infty\bigr) & j = k,\end{cases}
\]
and the discrete in-window threshold set $\tau_j := 4j/r^2 + \bigl(\sigma_{\mathrm{pp}}(-\Lap_{\cM_{k-j}(S^4_r)})\cap[0, 4/r^2)\bigr)$ (for $j = k$ the residual factor is a point and $\tau_k = \{4k/r^2\}$). For a compact interval $I \Subset W_j\setminus\tau_j$ write $E_I$ for the corresponding spectral projection. There exists $\epsilon_0 = \epsilon_0(I, j, r) > 0$ such that for all $0 < \epsilon < \epsilon_0$ the Laplace--Beltrami operator $-\Lap_{U_\epsilon^{(j)}}$ on the collar satisfies:
\begin{enumerate}\setlength{\itemsep}{2pt}
\item[\textup{(i)}] A strict Mourre estimate holds on $I$ with conjugate operator $A_j$ defined initially on $C_c^\infty(U_\epsilon^{(j)})$ by $A_j = \sum_{i=1}^j \tfrac12(\rho_i D_{\rho_i} + D_{\rho_i}\rho_i)\,\chi(\rho_i \ge \rho_0)$, where $\rho_i$ is the geodesic-radial coordinate on the $i$-th bubble end, $D_{\rho_i} = -i\partial_{\rho_i}$, and $\chi$ is a smooth cutoff supported in the ends; $A_j$ is essentially self-adjoint on $C_c^\infty(U_\epsilon^{(j)})$ (Nelson's analytic-vector theorem, Reed--Simon \cite[Thm.~X.39]{ReedSimonII}, the cutoff geodesic-dilation flow being complete). There is a constant $a_I > 0$ with
\[
E_I(-\Lap_{U_\epsilon^{(j)}})\,[-\Lap_{U_\epsilon^{(j)}},\,iA_j]\,E_I(-\Lap_{U_\epsilon^{(j)}})\;\ge\;a_I\,E_I(-\Lap_{U_\epsilon^{(j)}})\;+\;K_\epsilon,
\]
with $K_\epsilon$ compact.
\item[\textup{(ii)}] $A_j$ is of class $C^{1,1}(-\Lap_{U_\epsilon^{(j)}})$ in the sense of \cite[Def.~6.2.2]{ABMG1996}; in particular both iterated commutators $[-\Lap_{U_\epsilon^{(j)}}, iA_j]$ and $[[-\Lap_{U_\epsilon^{(j)}}, iA_j], iA_j]$ extend from $C_c^\infty$ to bounded operators $\mathcal{D}(-\Lap_{U_\epsilon^{(j)}})\to \mathcal{D}(-\Lap_{U_\epsilon^{(j)}})^*$ with norm $O(1)$ as $\epsilon\to 0$ (a fortiori $A_j$ is of class $C^2$, which is strictly stronger than $C^{1,1}$).
\item[\textup{(iii)}] Consequently \cite[Thm.~0.1]{Sahbani1997}, $-\Lap_{U_\epsilon^{(j)}}$ has empty singular continuous spectrum in $W_j\setminus\tau_j$; its eigenvalues in $W_j\setminus\tau_j$ have finite multiplicity and accumulate only at $\tau_j$; and a limiting absorption principle holds on each $I \Subset W_j\setminus(\tau_j\cup\sigma_{\mathrm{pp}})$, the boundary values $\langle A_j\rangle^{-s}(-\Lap_{U_\epsilon^{(j)}} - \lambda \mp i0)^{-1}\langle A_j\rangle^{-s}$ ($s>\tfrac12$) existing and being H\"older-continuous in $\lambda\in I$. In particular the spectrum in $W_j\setminus\tau_j$ is purely absolutely continuous off a discrete eigenvalue set.
\end{enumerate}
For $1\le j<k$ the window $W_j$ ends at the first residual channel $4(j+1)/r^2$, above which the bubble-only conjugate operator $A_j$ no longer yields a strict estimate (the residual factor $\cM_{k-j}(S^4_r)$ carries its own continuous spectrum from $4/r^2$ up, on which $A_j$ acts trivially); extending the spectral type past $4(j+1)/r^2$ would require enlarging $A_j$ to dilate the residual ends as well. For $j = k$ there is no residual factor and the statements hold on all of $(4k/r^2,\infty)$.
\end{theorem}

\begin{proof}
We work first on the exact product model $\cM_{k-j}(S^4_r)\times \bigtimes_{i=1}^j (\bbH^5_r\text{-end})$ and then add the $O(\epsilon|\log\epsilon|)$ metric perturbation.

\smallskip
\noindent\emph{Geodesic-polar reduction of a bubble end.} Near the Uhlenbeck boundary a bubble factor is asymptotic to $\bbH^5_r$, which we take in geodesic-polar coordinates $(\rho,\omega)\in(0,\infty)\times S^4$ about an interior basepoint,
\[
g_{\bbH^5_r} \;=\; d\rho^2 \;+\; r^2\sinh^2(\rho/r)\,d\Omega_{S^4}^2 ,
\]
the Uhlenbeck limit $s_i\to 0$ being $\rho_i\to\infty$ (indeed $\rho_i = r\log(r^2/s_i) + O(1)$, consistent with $y_i = e^{\rho_i/r} = r^2/s_i$ below). The cross-sections are round $4$-spheres of the \emph{compact} centre space $S^4$, so the transverse Laplacian $-\Lap_{S^4}$ has \emph{discrete} spectrum $\{\ell(\ell+3):\ell\ge 0\}$; this is the geodesic-polar foliation of $\bbH^5$ (adapted to the whole conformal boundary $S^4$ of instanton centres), not the horospherical one (adapted to a single boundary point). Decomposing $L^2(\bbH^5_r\text{-end})$ over the $-\Lap_{S^4}$ eigenspaces and conjugating each sector by the half-density $(r\sinh(\rho_i/r))^{2}$ reduces $-\Lap_{\bbH^5_r}$ to the half-line Schr\"odinger operators
\begin{equation}\label{eq:Hconj-ell}
H_\ell \;=\; -\partial_{\rho_i}^2 \;+\; \frac{4}{r^2} \;+\; \frac{2 + \ell(\ell+3)}{r^2\,\sinh^2(\rho_i/r)}\qquad\text{on } L^2\bigl((0,\infty),d\rho_i\bigr),
\end{equation}
with common threshold $4/r^2$ (the McKean bottom) and a centrifugal term that is \emph{short-range}, decaying like $e^{-2\rho_i/r}$ as $\rho_i\to\infty$. The Liouville computation of \eqref{eq:Hconj-ell} is verified symbolically in \cite{mourre-geodesic-script}.

\smallskip
\noindent\emph{Mourre identity for the geodesic dilation.} On each sector take the geodesic-dilation generator $A_{\rho_i} := \tfrac12(\rho_i D_{\rho_i} + D_{\rho_i}\rho_i)$, $D_{\rho_i}=-i\partial_{\rho_i}$ (leading coefficient $\rho_i\sim\log(1/s_i)$; this is the correct conjugate operator, whereas the na\"ive generator $\tfrac12(y_i\partial_{y_i}+\partial_{y_i}y_i)$ with $y_i=e^{\rho_i/r}$ is the \emph{translation} generator $r^{-1}\partial_{u_i}$ in $u_i=\log y_i$ and commutes with $-\partial_{\rho_i}^2$; the vanishing of that na\"ive commutator on the radial sector is recorded in \cite{mourre-cusp-script}). A direct computation, verified symbolically in \cite{mourre-geodesic-script}, gives the operator identity
\begin{equation}\label{eq:Hconj-mourre}
[H_\ell,\, iA_{\rho_i}] \;=\; 2\bigl(H_\ell - 4/r^2\bigr) \;+\; R_\ell(\rho_i),\qquad
R_\ell(\rho_i) = \frac{2\,(2+\ell(\ell+3))}{r^2\sinh^2(\rho_i/r)}\Bigl[\tfrac{\rho_i}{r}\coth\tfrac{\rho_i}{r} - 1\Bigr],
\end{equation}
where the remainder $R_\ell$ is short-range ($R_\ell(\rho_i)\to 0$ as $\rho_i\to\infty$, at rate $\rho_i\,e^{-2\rho_i/r}$), hence relatively $(-\partial_{\rho_i}^2)$-compact. Because the transverse spectrum is discrete and each $R_\ell$ decays, the assembled operator on the full end $\bbH^5_r\text{-end}$ satisfies
\begin{equation}\label{eq:cusp-mourre}
[-\Lap_{\bbH^5_r\text{-end},i},\,iA_{\rho_i}] \;=\; 2\bigl(-\Lap_{\bbH^5_r\text{-end},i} - 4/r^2\bigr) + K^{(i)}_{\mathrm{cpt}},
\end{equation}
with $K^{(i)}_{\mathrm{cpt}}$ relatively compact. This is the Froese--Hislop estimate for the Laplacian on a conformally compact (asymptotically hyperbolic) end \cite{FroeseHislop1989}; the compactness of $K^{(i)}_{\mathrm{cpt}}$ uses only the decay of $R_\ell$ and the discreteness of the $S^4$-transverse spectrum, and the cutoff $\chi(\rho_i\ge\rho_0)$ error is supported in the precompact slab $\{\rho_0\le\rho_i\le\rho_0+1\}\times S^4$, relatively compact by Rellich--Kondrachov on that compact-closure region.

\smallskip
\noindent\emph{Tensor assembly over the $j$ ends and the residual factor.} The conjugate operator $A_j = \sum_i A_{\rho_i}\,\chi(\rho_i\ge\rho_0)$ acts only on the bubble ends and is the identity on the residual factor $\cM_{k-j}(S^4_r)$; hence $[-\Lap_{\cM_{k-j}}\otimes 1,\,iA_j] = 0$ identically, and with $-\Lap_{\mathrm{prod}} = -\Lap_{\cM_{k-j}}\otimes 1 + \sum_i 1\otimes(-\Lap_{\mathrm{end},i})$,
\begin{equation}\label{eq:prod-mourre}
[-\Lap_{\mathrm{prod}},\,iA_j] \;=\; \sum_{i=1}^j 1\otimes\bigl[\,2(-\Lap_{\mathrm{end},i}-4/r^2) + K^{(i)}_{\mathrm{cpt}}\,\bigr].
\end{equation}
The spectrum of $-\Lap_{\mathrm{prod}}$ is $\sigma(-\Lap_{\cM_{k-j}}) + \sum_i[4/r^2,\infty)$. Fix a compact $I\Subset W_j\setminus\tau_j$. The window is what makes the bubble-only operator $A_j$ suffice: for $E\in W_j$ with $E < 4(j+1)/r^2$, any decomposition $E = E_{\mathrm{res}} + \sum_i E_i$ with $E_i\ge 4/r^2$ (each bubble end has bottom $4/r^2$) forces the residual energy $E_{\mathrm{res}} = E - \sum_i E_i < 4/r^2$, i.e.\ below the residual factor's own essential threshold; hence on $E_I$ the residual factor sits in its \emph{point} spectrum $\sigma_{\mathrm{pp}}(-\Lap_{\cM_{k-j}})\cap[0,4/r^2)$, a discrete set, and $\sum_i E_i = E - E_{\mathrm{res}}$ is bounded away from $4j/r^2$ by $\mathrm{dist}(I,\tau_j)$. Thus at least one bubble end carries energy bounded away from $4/r^2$, and by the tensor-sum Mourre calculus \cite[\S 8.3.4]{ABMG1996}, \eqref{eq:prod-mourre} yields $E_I[-\Lap_{\mathrm{prod}},iA_j]E_I \ge a_I E_I + (\text{compact})$ with $a_I>0$ depending on $\mathrm{dist}(I,\tau_j)$. (For $E\ge 4(j+1)/r^2$ the residual factor can carry a.c.\ energy $\ge 4/r^2$ with every bubble at threshold, where $A_j$ gives no positivity; this is the reason for the window.)

\smallskip
\noindent\emph{Metric perturbation.} With $g_{U_\epsilon^{(j)}} = g_{\mathrm{prod}} + h$, $\|h\|_{g_{\mathrm{prod}}}\le C\epsilon|\log\epsilon|$ (Theorem~\ref{thm:M_k-collar-conditional}), one has $[-\Lap_{U_\epsilon^{(j)}},iA_j] = [-\Lap_{\mathrm{prod}},iA_j] + R^{(1)}_\epsilon$ with $\pm R^{(1)}_\epsilon \le C\epsilon|\log\epsilon|^3\,(-\Lap_{\mathrm{prod}}+1)$ as forms, by Lemma~\ref{app:lem:iterated} at $n=1$ (the geodesic-dilation coefficient $\rho_i\sim|\log\epsilon|$ contributes an extra logarithmic factor per order relative to the scale-derivative case, which is harmless). For $\epsilon<\epsilon_0(I)$ this preserves a strict Mourre estimate on $I$ with constant $a_I/2$, proving (i).

\smallskip
\noindent\emph{Regularity (ii).} On the model \eqref{eq:Hconj-ell} the second commutator iterates to $[[H_\ell,iA_{\rho_i}],iA_{\rho_i}] = 4(-\partial_{\rho_i}^2) + \widetilde R_\ell$ with $\widetilde R_\ell$ short-range, so the unperturbed second commutator is bounded relative to $-\Lap_{\mathrm{prod}}+1$; the perturbative part is $R^{(2)}_\epsilon = O(\epsilon|\log\epsilon|^5)\to 0$ by Lemma~\ref{app:lem:iterated} at $n=2$. Both iterated commutators are thus $-\Lap_{U_\epsilon^{(j)}}$-bounded with $O(1)$ norm, giving $A_j\in C^2(-\Lap_{U_\epsilon^{(j)}})\subsetneq C^{1,1}(-\Lap_{U_\epsilon^{(j)}})$, the hypothesis of Sahbani \cite[Hyp.~H1--H3, p.~411]{Sahbani1997}.

\smallskip
\noindent Statement (iii) is the conclusion of \cite[Thm.~0.1]{Sahbani1997}: on $I$ the limiting absorption principle holds off the eigenvalues, $\sigma_{\mathrm{sc}}\cap I=\emptyset$, and eigenvalues in $I$ are finite with finite multiplicity; letting $I$ exhaust $W_j\setminus\tau_j$ gives the stated form, with eigenvalues accumulating only at $\tau_j$.
\end{proof}

\begin{remark}[Spectral type off the thresholds]\label{rmk:AC}
Theorem~\ref{thm:M_k-collar-spectral-type} gives, on the first-channel window $W_j\setminus\tau_j$, purely absolutely continuous spectrum together with a discrete set of finite-multiplicity embedded eigenvalues; the singular continuous spectrum is empty there. For $1\le j<k$ the window is the bounded interval $(4j/r^2, 4(j+1)/r^2)$ below the first residual channel; for $j=k$ there is no residual factor and the description is clean on all of $(4k/r^2,\infty)$. Whether embedded eigenvalues occur is not decided by Mourre theory; on the exact model $\bbH^5_r$ (the case $k=j=1$) the spectrum is purely absolutely continuous with none, consistent with Theorem~\ref{thm:M1-spectrum}.
\end{remark}
...
\]
acting on $L^2((0,\infty), J(s)\,ds)$ where $J(s)$ is the codimension-1 volume density and $V_{\mathrm{true}}(s) = \tfrac14(J'/J)^2 + \tfrac12(J'/J)'$ is the Liouville potential. The principal-orbit radius interpolates between $r_{\mathrm{orbit}}(s) \sim s$ at the orbifold tip $s\to 0$ and $r_{\mathrm{orbit}}(s) \sim r\sinh(s/r)$ at the cusp $s\to\infty$ \cite{m2-isotypic-decomposition}. At the end, $V_{\mathrm{true}}(s) \to 4/r^2$ (McKean rate on the $\bbH^5_r$ horocycle, independently of the bare-vs-projected normalization discussion of \cite{m2-closure-higher-order}) and $C_\ell/r_{\mathrm{orbit}}(s)^2 \to 0$ exponentially; at the tip, the Casimir potential dominates as $C_\ell/s^2$, repulsive. By the Bargmann criterion (Reed--Simon IV \cite{ReedSimonIV}, Thm.~XIII.9),
\[
...
For the \emph{geodesic-dilation} field $\rho_i\partial_{\rho_i}$ actually used as the Mourre conjugate operator in \S\ref{sec:mourre}, note $\rho_i = r\log(r^2/s_i) + O(1)$ so that $\rho_i\partial_{\rho_i} = -\log(r^2/s_i)\,(s_i\partial_{s_i})$ on the end; each application therefore carries an extra scalar factor $\log(r^2/s_i)$. Since $s^2\log^{2n+1}(r^2/s)$ is increasing on $(0,\epsilon]$ for $\epsilon$ small (the positive power of $s$ dominates the logarithmic growth), its supremum over the end is attained at the neck $s\sim\epsilon$, giving
\begin{equation}\label{eq:iterated-h-bound-geodesic}
\Bigl\|\Bigl(\sum_{i=1}^j \rho_i\partial_{\rho_i}\Bigr)^{\!n} h\Bigr\|_{g_{\mathrm{prod}}} \;\le\; C_n\,\epsilon\,(1+|\log\epsilon|)^{2n+1},
\end{equation}
one extra logarithmic factor per order relative to \eqref{eq:iterated-h-bound}.
\end{lemma}

\begin{proof}
By Lemmas~\ref{lem:cross-term} and \ref{lem:OD}, every cross-block entry of $h$ in the gauge-fixed basis is a known function of the bubble parameters with explicit scale dependence, and it is these closed forms, not any homogeneity normalization of the individual tangent vectors, that we differentiate. Write $\epsilon = \max_i s_i$; the separation $R$ is $\Theta(1)$ by the floor $\rho_0$. Each bubble--bubble entry has the form
\[
\langle \hat v_\alpha^{(i)},\hat v_\beta^{(j')}\rangle_{L^2} \;=\; \frac{s_i s_{j'}}{R^2}\,\phi_{\alpha\beta}\bigl(\log(s_i/R),\,\log(s_{j'}/R)\bigr),
\]
with $\phi_{\alpha\beta}$ of degree $\le 1$ in each logarithmic argument (uniformly in the order of the $(s/R)^2$ expansion): for the scale--scale entry the closed form \eqref{eq:lemma-ct-elementary} contributes its $\operatorname{arccosh}(\Sigma/2 s_i s_{j'})$ term, which is the hyperbolic distance $d = -\log(s_i/R) - \log(s_{j'}/R) + O(1)$ appearing linearly (so degree $1$, entering at order $(s/R)^4$); the scale--position and gauge-fixed position--position entries carry the borderline logarithm of \eqref{eq:OD-horiz} at leading order. Each bubble--background entry has the form $\langle a,\hat v_\alpha^{(i)}\rangle_{L^2} = (s_i^2/R)\,\psi_\alpha(\log(s_i/R))\,\|a\|_{L^2}\|\hat v_\alpha^{(i)}\|_{L^2}$ with $\psi_\alpha$ of degree $\le 0$ (Proposition~\ref{app:prop:bg}). In every case the entry is $s_i^{p}s_{j'}^{q}R^{-c}$ times a bounded-degree polynomial in the logarithms, with $p, q\ge 1$.

\emph{Effect of the scale dilation.} Since $s_i\partial_{s_i}\log(s_i/R) = 1$, applying $s_i\partial_{s_i}$ to a term $\Psi = s_i^{p}s_{j'}^{q}R^{-c}\,\phi(\log(s_i/R),\log(s_{j'}/R))$ gives
\[
s_i\partial_{s_i}\Psi \;=\; p\,\Psi \;+\; s_i^{p}s_{j'}^{q}R^{-c}\,(\partial_1\phi):
\]
the scale powers $(p,q)$ are unchanged and the polynomial degree of $\phi$ rises by at most one, with no inverse power $1/s_i$ produced. Iterating, $(s_i\partial_{s_i})^n\Psi = s_i^{p}s_{j'}^{q}R^{-c}\,\phi_n$ with $\deg\phi_n\le\deg\phi + n$, so
\begin{equation}\label{eq:iterated-scale-derivative-cross}
\bigl|(s_i\partial_{s_i})^n \langle\hat v_\alpha^{(i)},\hat v_\beta^{(j')}\rangle_{L^2}\bigr| \;\le\; C_n\,\frac{s_i s_{j'}}{R^2}\,\bigl(1+|\log(\min(s_i,s_{j'})/R)|\bigr)^{1+n},
\end{equation}
and likewise $\bigl|(s_i\partial_{s_i})^n\langle a,\hat v_\alpha^{(i)}\rangle_{L^2}\bigr| \le C_n\,(s_i^2/R)\,(1+|\log\epsilon|)^{n}\,\|a\|_{L^2}\|\hat v_\alpha^{(i)}\|_{L^2}$ for the bubble--background entries. This uses only the established closed forms of Lemmas~\ref{lem:cross-term} and \ref{lem:OD}.

\emph{Conversion to the cusp dilation.} On the end of the $i$-th bubble factor, $y_i\partial_{y_i} = -s_i\partial_{s_i}$ (the cusp coordinate is $y_i \sim r^2/s_i$, so $\log y_i = 2\log r - \log s_i$, giving $\partial/\partial\log y_i = -\partial/\partial\log s_i$). Iterated $(y_i\partial_{y_i})^n$ thus produces $(-1)^n (s_i\partial_{s_i})^n$ on the cross-block entries. The sum over $i$ in $A_j$ yields a multinomial expansion of $(\sum_i y_i\partial_{y_i})^n$ into products of $(y_{i_1}\partial_{y_{i_1}})\cdots(y_{i_n}\partial_{y_{i_n}})$ acting on a single cross-block entry $\langle\hat v^{(i)}, \hat v^{(j')}\rangle$, which is nonzero only when $\{i_1,\dots,i_n\}\subseteq\{i,j'\}$ (each $\hat v^{(i)}$ depends only on the $i$-th bubble's parameters). Each such product is controlled by \eqref{eq:iterated-scale-derivative-cross} with the same overall rate.
...
\bigl|\bigl\langle\bigl(\mathrm{ad}_{iA_j}^{(n)}(-\Lap_{U_\epsilon^{(j)}}) - \mathrm{ad}_{iA_j}^{(n)}(-\Lap_{\mathrm{prod}})\bigr)\phi,\phi\bigr\rangle\bigr| \;\le\; C_n\,\epsilon\,(1+|\log\epsilon|)^{2n+1}\,\langle(-\Lap_{\mathrm{prod}}+1)\phi,\phi\rangle
\end{equation}
for $\phi \in C_c^\infty(U_\epsilon^{(j)})$, where $A_j$ is the geodesic-dilation conjugate operator of \S\ref{sec:mourre}. In particular for $n=2$ the right-hand side is $O(\epsilon|\log\epsilon|^5)$, which is the $C^{1,1}$ (in fact $C^2$) regularity input for Theorem~\ref{thm:M_k-collar-spectral-type}(ii).
\end{lemma}
...
\]
with $L_h$ a second-order operator whose coefficients are smooth functions of $h$ and $\nabla^{\mathrm{prod}}h$, polynomial in the coordinates of $h$ (and $g_{\mathrm{prod}}^{-1}$, which is bounded uniformly on the cusp). The geodesic-dilation generator $A_j$ has the explicit commutator structure \eqref{eq:cusp-mourre}--\eqref{eq:prod-mourre} of \S\ref{sec:mourre} against $-\Lap_{\mathrm{prod}}$ (cusp-supported, modulo the relatively compact $K_{\mathrm{cpt}}^{(i)}$ collecting the cutoff $\chi$ error, the short-range Casimir contribution from sectors $\ell\ge 1$, and the half-density conjugation residual; cf.\ Froese--Hislop \cite{FroeseHislop1989}).
...
\]
where $\mathcal{L}_{A_j}$ is the Lie derivative along the geodesic-dilation vector field $A_j = \sum_i \rho_i\partial_{\rho_i}$ (with cutoff). The lower-order term $[L_h, iA_j]_{\mathrm{l.o.t.}}$ is a first-order differential operator with coefficients of the same form as $h, \nabla^{\mathrm{prod}}h$ (paired with the bounded coefficients of $A_j$), of the same operator-norm size as $L_h$ itself. Iterating $n$ times produces $L_{\mathcal{L}_{A_j}^n h}$ plus lower-order analogues with coefficients in $(\mathcal{L}_{A_j}^k h)_{0\le k\le n}$.

By Lemma~\ref{app:lem:iterated-h} in its geodesic-dilation form \eqref{eq:iterated-h-bound-geodesic}, $\|\mathcal{L}_{A_j}^k h\|_{g_{\mathrm{prod}}}\le C_k\epsilon(1+|\log\epsilon|)^{2k+1}$ pointwise on the ends. Substituting into the operator-norm decomposition $L_h = (g^{-1} - g_{\mathrm{prod}}^{-1})\partial^2 + B^k\partial_k$, applying the same Hardy-inequality manoeuvre of \S\ref{app:sec:laplacian} to absorb the first-order term $B^k\partial_k$ against $-\Lap_{\mathrm{prod}}+1$, and bounding the $\partial^2$ term by $\langle(-\Lap_{\mathrm{prod}}+1)\phi,\phi\rangle$ via standard $H^2$-elliptic regularity, we obtain
\[
|\langle L_{\mathcal{L}_{A_j}^k h}\phi,\phi\rangle| \;\le\; C\,\|\mathcal{L}_{A_j}^k h\|_{g_{\mathrm{prod}}}\,\langle(-\Lap_{\mathrm{prod}}+1)\phi,\phi\rangle \;\le\; C_k\,\epsilon(1+|\log\epsilon|)^{2k+1}\,\langle(-\Lap_{\mathrm{prod}}+1)\phi,\phi\rangle.
\]
Summing over $k\le n$ (with multinomial constants absorbed into $C_n$) gives \eqref{eq:iterated-commutator-bound}.

For $n=1$ the bound is $O(\epsilon|\log\epsilon|^3)$ (the single-commutator input of Theorem~\ref{thm:M_k-collar-spectral-type}(i)); the coarser scale-derivative estimate $O(\epsilon|\log\epsilon|^2)$ suffices for the essential-bottom identification of Theorem~\ref{thm:M_k-collar-conditional}, which uses only the operator-norm comparison and not the geodesic dilation. For $n=2$ the bound is $O(\epsilon|\log\epsilon|^5)$, which tends to $0$ as $\epsilon \to 0$, so the iterated commutator is uniformly bounded relative to $-\Lap_{\mathrm{prod}}+1$ and in particular relative to $-\Lap_{U_\epsilon^{(j)}}+1$ (since the two are mutually bounded by \S\ref{app:sec:laplacian}). This is precisely the $C^{1,1}(-\Lap_{U_\epsilon^{(j)}})$-regularity hypothesis of Sahbani \cite[Hyp.~H1--H3]{Sahbani1997}; in fact, since the bound is $O(\epsilon|\log\epsilon|^5)\to 0$, we obtain the stronger $C^2(-\Lap_{U_\epsilon^{(j)}})$ regularity.
\end{proof}

\begin{remark}[Why the geodesic dilation]\label{rmk:iterated-honest}
The bounds \eqref{eq:iterated-h-bound}--\eqref{eq:iterated-h-bound-geodesic} do not gain inverse-bubble-scale factors $1/s_i$ that would conflict with the $\epsilon$ decay: $s_i\partial_{s_i}$ acting on the scale-homogeneous BPST profile produces tangent vectors of the same scale class (Lie derivative along the conformal scale field), and each geodesic dilation $\rho_i\partial_{\rho_i}$ adds only a scalar factor $\log(r^2/s_i)$, hence at most a logarithm per order. The geodesic dilation, and not the scale derivative, is the correct \emph{conjugate operator}: on the half-line model \eqref{eq:Hconj-ell} the scale-derivative field $y_i\partial_{y_i} = -s_i\partial_{s_i}$ is the translation generator $\partial_{u_i}$ ($u_i=\log y_i$), which commutes with $-\partial_{u_i}^2$ and gives no Mourre estimate, whereas the geodesic dilation $\rho_i\partial_{\rho_i} = u_i\partial_{u_i}$ (up to $O(1)$) is the dilation generator, whose commutator with $-\partial_{u_i}^2$ is $2(-\partial_{u_i}^2)$ (the Froese--Hislop mechanism, \eqref{eq:Hconj-mourre}).
\end{remark}
...
\bibitem{lemma-schwinger-script} V.\ Bonfioli, Schwinger-parametrized closed form for the BPST scale-derivative cross-term, with uniform-box Monte Carlo cross-check: \texttt{verification/lemma\_ct\_montecarlo.py}.
...
\bibitem{mourre-geodesic-script} V.\ Bonfioli, symbolic verification of the geodesic-polar Froese--Hislop reduction for Theorem~\ref{thm:M_k-collar-spectral-type}: \texttt{verification/mourre\_geodesic\_polar.py}. Checks the half-density Liouville reduction \eqref{eq:Hconj-ell}, the Mourre identity \eqref{eq:Hconj-mourre} with short-range remainder $R_\ell$, the vanishing of the na\"ive translation-generator commutator, and the threshold/positivity crossover, all in \texttt{sympy} (exact) with a numerical crossover check.

\bibitem{mourre-cusp-script} V.\ Bonfioli, Sage symbolic check that the unconjugated/translation-generator commutator vanishes on the radial sector of $\bbH^5_r$ (the reason the geodesic dilation, not the scale derivative, is the correct conjugate operator): \texttt{verification/mourre\_estimate\_cusp.sage}.
