...
\begin{equation}\label{eq:geom-constants}
  \Vol(\cM_1,g_{L^2}) = 943.393\,r^5 = 9.68486\,\pi^4 r^5,\quad
  \Vol(\partial) = \tfrac{2}{3}\pi^6 r^4,\quad
  \operatorname{diam_{rad}} = 3.7437\,r,
\end{equation}
and strictly positive, non-constant sectional curvature (isotropic value $\approx 0.2533\,r^{-2}$ at the center, decreasing toward the boundary), in agreement with \cite[Cor.~C]{GP1987}. The volume is not a recognizable closed form; the boundary $4$-volume is exactly $\tfrac23\pi^6r^4$ because $g(0)=\pi^2/2$ exactly.
...
\end{equation}
with regularity at the center $x=1$ (where $u\sim(1-x)^k$) and the chosen elliptic condition at the boundary $x=0$. We solve \eqref{eq:SL} by linear finite elements in two discretizations that bracket the spectrum. The conforming, consistent-mass scheme is Rayleigh--Ritz, so it bounds every eigenvalue from above; the lumped-mass scheme converges from below. Both are second order. At $2.4\times10^4$ elements the two bracket each eigenvalue quoted here monotonically from either side, with enclosure width below $10^{-7}$, for instance
\[
  \lambda_0(-\Lap_D)\in[0.8203176,\,0.8203177],\qquad
  \lambda_1(-\Lap_N)=0.9988386\ \ (\text{width}<10^{-9}).
\]
The upper bound is rigorous. The lower bound rests on the standard underestimation property of mass lumping, which we verify numerically to be operative here (the lumped values increase monotonically under refinement), so it is not certified; eigenvalues are accordingly reported as enclosures rather than as proved values. The coefficients \eqref{eq:f}--\eqref{eq:g} are only $C^1$ at $x=0$, carrying an $x^2\ln x$ term that the Neumann modes feel and the Dirichlet modes, vanishing there, do not.
...
\[
  \lambda_0(-\Lap_D)\;=\;0.8203176\,r^{-2}\quad(k=0).
\]
...
\[
  \lambda_1(-\Lap_N)\;=\;0.9988386\,r^{-2}\quad(k=1,\ \text{multiplicity }5,\ \text{vector representation of }\SO(5)),
\]
which sits below the first radial ($k=0$) overtone $2.0355386\,r^{-2}$. The low eigenvalues $\lambda\,r^2$ are:
\begin{center}\small
